...
func check_condition(blackboard: Blackboard):
	...
	match comp_type:
		StatCompType.GREATER:
			result_type = StrategyCardNode.NodeResultType.SUCCESS if off_player_stat - def_player_stat >= threshold else StrategyCardNode.NodeResultType.FAILURE
		StatCompType.LESS:
			result_type = StrategyCardNode.NodeResultType.SUCCESS if def_player_stat - off_player_stat >= threshold  else StrategyCardNode.NodeResultType.FAILURE
		StatCompType.EQUAL:
