...
func show_strategy_card(sc: StrategyCard, callable: Callable):
	...
	var on_slide_in_complete = func slide_in_complete():
		...
		fade_tween.tween_property(strategy_card, "modulate:a", 0, 0.25).set_delay(0.5)
	...
	slide_in.tween_property(strategy_card, "global_position", Vector2(prev_x, prev_y), 0.5)
