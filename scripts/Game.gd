...
func process_matchup_stats(all_stats: Dictionary, side: Side, assists_used: int):
	if side == Side.PLAYER:
		...
		opp_matchup_bp_card.marker.copy_from_marker(matchup_container.cpu_card.marker)
		...
		opp_matchup_bp_card.marker.copy_from_marker(matchup_container.player_card.marker)
